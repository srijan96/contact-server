...
io.on('connection', function(socket) {
  ...
  socket.on('add user', function(user){
    ...
    if(user == "") {
      socket.emit('chat message', "Add user failed for " + user);
  ...
  socket.on('handle qa', function(user, question, ans){
    ans = ans.toLowerCase();
      ...
      socket.emit('chat message', "added question \nQ: " + question + "\nA: " + ans);
  ...
  //Testing MongoDB
  socket.on("save result", function(user, data){
    console.log("saving the content in mongodb");
    dao.saveData({name:user, question:data.question, answer:data.answer}, 
      function(err){
        socket.emit(err);
      });
  })
  ...
  socket.on("fetch data", function(){
    console.log("Fetching all data from mongo");
    data = dao.getData(function(){
      console.log("Probably wont reach here.");
    })
